...
| POST | /cards| Add a new card whose attributes have to be specified in the body, of type JSON, (the id is automatically generated). To add a card a password is needed bacause is is not intended for a non-admin user to create traditional tarot cards. If it is correctly added, it returns “201 Created” with a reference to the URI and the new Card body|
| PUT | /cards  | Updates the card with the attributes specified in the body (card id must be included). To update a card a password is needed bacause is is not intended for a non-admin user to update traditional tarot cards. If the card does not exist, a “404 Not Found” error is returned. If it is correctly done it returns “204 No Content”. |
| DELETE | /cards/{id}  |  Deletes the card with the same id. If the card does not exist it returns “404 Not Found” error. If the card is correctly deleted it returns “204 No Content”.|
